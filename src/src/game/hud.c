...
s8 warningState = 1;
u8 warningTimer = 0;
s8 subliminalState = 1;
u16 subliminalTimer = 0;
u16 subliminalIntensity = 0;
u16 subliminalMessageID = 0;
...
/**
 * DEBUG! Render AI gen values
 */
void render_hud_ai_gen(void) {
    print_text_fmt_int(24, 20, "SEED %0d", aiModeSeed[0]);
    print_text_fmt_int(24, 38, "RNG1 %0d", aiModeSeed[1]);
    print_text_fmt_int(24, 56, "RNG2 %0d", aiModeSeed[2]);
    print_text_fmt_int(24, 74, "RNG3 %0d", aiModeSeed[3]);
}
...
void render_hud_keys(void) {
    if (gMarioState->numKeys == 0) {
        print_text(GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(HUD_STARS_X), (HUD_TOP_Y - 18), ".");
        print_text(GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(HUD_STARS_X) + 15, (HUD_TOP_Y - 18), ".");
        print_text(GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(HUD_STARS_X) + 30, (HUD_TOP_Y - 18), ".");
    } else if (gMarioState->numKeys == 1) {
        print_text(GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(HUD_STARS_X), (HUD_TOP_Y - 18), "/");
        print_text(GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(HUD_STARS_X) + 15, (HUD_TOP_Y - 18), ".");
        print_text(GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(HUD_STARS_X) + 30, (HUD_TOP_Y - 18), ".");
    } else if (gMarioState->numKeys == 2) {
        print_text(GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(HUD_STARS_X), (HUD_TOP_Y - 18), "/");
        print_text(GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(HUD_STARS_X) + 15, (HUD_TOP_Y - 18), "/");
        print_text(GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(HUD_STARS_X) + 30, (HUD_TOP_Y - 18), ".");
    } else {
...
// Warning indicator
void render_warning(void) {
//    print_text_fmt_int(20, 20, "%00000d", subliminalIntensity);
//    print_text_fmt_int(20, 36, "%00000d", subliminalTimer);
//    if (warningState == 1) {
//        warningTimer = warningTimer + 1;
//        if (warningTimer <= 20) {
//            print_text_centered(GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(HUD_STARS_X) + 24, 20, "DEMO");
//        }
//		if (warningTimer >= 40) {
//			warningTimer = 0;
//		}
...
void render_subliminal(void) {
    subliminalTimer++;
    if (subliminalTimer == subliminalIntensity) {
        switch (subliminalMessageID) {
            case 0:
                print_text_centered(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, "YOU ARE NOT WORTHY");
                break;
            
            case 1:
                print_text_centered(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, "PUT DOWN THE CONTROLLER");
                break;

            case 2:
                print_text_centered(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, "TURN OFF THE CONSOLE");
                break;

            case 3:
                print_text_centered(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, "I CAN SEE YOU");
                break;

            case 4:
                print_text_centered(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, "YOU ARE NOT WELCOME");
                break;
            return;
        }
        subliminalMessageID = random_u16() & 0x3;
        subliminalTimer = 0;
        subliminalIntensity = random_u16() & 0x1328;
    }
}
